"""
Module with metrics and loss functions for evaluating model quality.
"""
...
def masked_bce_loss(
    logit_map: torch.Tensor, target: torch.Tensor, ignore_class: int = 2
) -> torch.Tensor:
    """
    ...
    if valid_mask.any():
        ...
        return torch.tensor(0.0, device=logit_map.device, requires_grad=True)


def masked_mse_loss(
    prediction: torch.Tensor, target: torch.Tensor, ignore_class: int = 2
) -> torch.Tensor:
    """
